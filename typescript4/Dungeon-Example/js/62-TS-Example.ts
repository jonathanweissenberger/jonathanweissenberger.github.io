...
interface Monster {
    ...
    monsterImage : string ; //Interface Erweiterung um 2 weitere Felder (Bilder und Rüstung)
    monsterArmor : string ;
    //monsterLevel : number;
}
...
let playerXP : number = 0 ;                             //Fehler gefunden No1 Variablen                           // Stellt die gesammelte Erfahrung des Spielers dar.
...
let prefix : string[] = ["Wald-", "Seuchen-", "Uralte(s) ", "Gift-", "Brennende(s) ", "Kniescheibenzertrümmernde(s) ", "Durchtriebene(s) ", "Garstige(s) ","Schleim-", "Kanibalen ", "Elite "]; // (length = 6, da 6 Einträge. Von 0-5.) Erweiterung der 3 Namensteile um jeweils 5 Namen
let monsterName : string[] = ["Ratte ", "Nagetier ", "Ungeziefer ", "Frosch ", "Ghul ", "Lappen ", "Schnecke ", "Waldfeh "]; // length = 3, da 3 Einträge. Von 0-2.
let suffix : string[] = [" des Verderbens", " aus der Hölle", " der Lethalität", " mit Rheuma", " der Redundanz", " der Zerberstung", "mit Fischaugen", "der Trostlosigkeit", "des Vertrauens", "ohne Skrupel", "aus Mordor"]; // length = 6, da hier 6 Einträge sind. Von 0-5.

let monsterSrc : string[] = imagePush(); 
let monsterArmor : string [] = ["iron shield", "enchanted leather gloves", "plated steel helmet"]; //verschiedene Rüstungsoptionen
...
function generateMonster()
{
    ...
    let newMonsterArmor : string = generateMonsterArmor();
    ...
    let newMonster : Monster = {                                        // Monster wird erstellt.
        ...
        //monsterMoney : 0,     Fehler gefunden No1 Funktion
    }
...
function monsterGenerateHTML(monsterCount : number)
{
    ...
    let monsterHealthPoints : HTMLElement = document.createElement("p");        // Generiere einen <p>
    ...
    let monsterExperience : HTMLElement = document.createElement("p");        // Generiere einen <p>
    ...
    let monsterArmor : HTMLElement = document.createElement("p");
...
function generateMonsterArmor() : string
{
    let rngNumber : number = getRNGNumber(monsterSrc.length);
    let generateMonsterArmor : string = monsterArmor[rngNumber];
...
/*function generateMonsterLevel() : number{
    ...
    let hund : number = Math.floor(Math.random() * 11)
    let generateMonsterLevel : number = monsterLevel[hund];
    return generateMonsterLevel;
}*/
}
